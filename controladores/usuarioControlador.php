	...
	class usuarioControlador extends usuarioModelo{
		...
		public function paginador_usuario_controlador($pagina,$registros,$privilegio,$id,$url,$busqueda){
			...
			if(isset($busqueda) && $busqueda!=""){
				...
				$consulta="SELECT SQL_CALC_FOUND_ROWS * FROM usuario WHERE 	usuario_id!='$id' AND usuario_id!='1' ORDER BY usuario_nombre ASC LIMIT $inicio,$registros"; /* Esto nos permite contar cuantos registrso hay en la base de datos según las consultas que se hagan*/
			}
			...
			$Npaginas=ceil($total/$registros); /*  ceil función de php que sirve para redondear un valor númerico*/
			...
			if($total>=1 && $pagina<=$Npaginas){
				$tabla.='<p class="text-right">  Mostrando Usuarios del '.$reg_incio.' al '.$reg_final.' cantidad total de: '.$total.' usuarios </p>';
		...
		public function eliminar_usuario_controlador(){
			...
			if($check_prestamos->rowCount()>0){
				$alerta=[
					"Alerta"=>"simple",
					"Titulo"=>"Ocurrió un error inesperado",
					"Texto"=>"No podemos eliminar este usuario debido a que tiene prestamos asociados, recomendamos sehabilitar el usuario si ya no será utilizado.",
					"Tipo"=>"error"
				];
				echo json_encode($alerta);
				exit();
